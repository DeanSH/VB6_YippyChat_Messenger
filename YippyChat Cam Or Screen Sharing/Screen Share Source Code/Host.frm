...
Begin VB.Form Host 
   ...
   Begin HostDesktop.CamSocket CamSocket 
      Height          =   375
      Left            =   10080
      TabIndex        =   7
      Top             =   1440
...
If Dir$(App.Path & "\Files\desktop.bmp") = "" Then Exit Sub
...
MsgBox "Error!! YippyChat DesktopHost.exe is allready Running!" & vbCrLf & "Running 2 Copies at once is Not Allowed, Please Close it First to Start it again Fresh! If unable to locate, open Task Manager and Force it Closed in there!", , "Error! DesktopHost.exe is allready Running!"
DoEvents
End
Exit Sub
End If
...
End If
   ...
   'Select Case GetINI("Method2")
    ...
    'Case "1"
      ...
      'WallPath = GetWallPaperPathAnConvert
      'DoEvents
      'Debug.Print WallPath
      DesktopIconsHide
      StatusBar1.Panels(1).Text = "Host: Loading Desktop..."
      Desktop = True
      Dim TheQ As String
      TheQ = Val(GetINI("Quality"))
      If TheQ > 60 Or TheQ < 30 Then
      TheQ = "55"
      WriteINI "Quality", TheQ
      End If
      HostSetup.sQuality.Value = TheQ
      HostSetup.sQuality.ToolTipText = HostSetup.sQuality.Value
      Me.Visible = False
      'Wsize = Screen.Width / 15
      'Hsize = Screen.Height / 15
      Wsize = Screen.Width / Screen.TwipsPerPixelX
      Hsize = Screen.Height / Screen.TwipsPerPixelY
      Debug.Print Wsize
      Debug.Print Hsize
      Sratio = (100 / Wsize) * Hsize
...
If Desktop = False Then
    ...
    DoJPG
    DoEvents
    CamSocket.ConnectServer TheIP, "3661"
    'CamSocket.ConnectServer "192.161.59.152", "3660"
    Timer1 = True
    cmdHost_Click
...
picCapture2.Height = (picCapture2.Width / 100) * Sratio
...
'On Error Resume Next
  ...
  DoEvents
  'If Dir$(App.Path & "\temp.bmp") <> "" Then SetWallPaper App.Path & "\temp.bmp"
  'DoEvents
  DesktopIconsShow
  Unload HostSetup
  DoEvents
  'If Dir$(App.Path & "\temp.bmp") <> "" Then Kill App.Path & "\temp.bmp"
  If Dir$(App.Path & "\Files\desktop.bmp") <> "" Then Kill App.Path & "\Files\desktop.bmp"
  If Dir$(App.Path & "\Files\desktop.jpg") <> "" Then Kill App.Path & "\Files\desktop.jpg"
  'deletes sending
  End
End Sub
...
Private Sub SDIs_Click()
If SDIs.Checked = True Then
SDIs.Checked = False
DesktopIconsHide
Else
SDIs.Checked = True
...
Dim s As String
  ...
  BMPtoJPG App.Path & "\Files\desktop.bmp", App.Path & "\Files\desktop.jpg", HostSetup.sQuality.Value
  
  If HostSetup.Check1.Value = 1 Then
  Pause2 "0.01"
  picCapture2.Picture = LoadPicture(App.Path & "\Files\desktop.jpg")
  Else
  'picCapture2.Picture = Pic3.Picture
  End If
  'converts bmp to jpg, shows jpg
  Open App.Path & "\Files\desktop.jpg" For Binary As #1
  sFile2 = Space(LOF(1))
  Get #1, , sFile2
  Close #1
  'DoEvents
  'Debug.Print Len(sFile2)
  'Debug.Print FileLen(App.Path & "\Files\desktop.jpg")
    'stores file
  sFile3 = sFile2
  Pause2 "0.01"
  Timer1 = True
