...
Public Function ChangeUser(Name As String, Pass As String, Info As String) As Boolean
On Error GoTo Exist
Dim TmpPass As String
TmpPass = Get_Name_Info(Name, "Password")
DoEvents
If TmpPass = Pass Then
Set_Name_Info Name, "Password", Info
DoEvents
TmpPass = Get_Name_Info(Name, "Password")
DoEvents
If TmpPass = Info Then
ChangeUser = True
Else
ChangeUser = False
End If
...
TmpData = ""
intE = FreeFile
If User = "" Then GoTo Error
If IsAvail(User) = True Then GoTo Error
...
LettersAllowed = "0ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890"
If InStr(1, LettersAllowed, Right(Name, 1)) > 0 Then

LettersAllowed = "_ABCDEFGHIJKLMNOPQRSTUVWXYZ.abcdefghijklmnopqrstuvwxyz1234567890_"
